...
AnovaRepeatedMeasuresWrapper <- function(
          data = NULL,
   ...
   defaultArgCalls <- formals(jaspAnova::AnovaRepeatedMeasuresWrapper)
   ...
   optionsWithFormula <- c("betweenModelTerms", "marginalMeanTerms", "postHocTerms", "restrictedMarginalMeanTerms", "withinModelTerms")
   for (name in optionsWithFormula) {
      if ((name %in% optionsWithFormula) && inherits(options[[name]], "formula")) options[[name]] = deparse1(options[[name]])   }

   if (jaspResultsCalledFromJasp()) {
      result <- list("options" = options, "analysis"="jaspAnova::AnovaRepeatedMeasures")
      result <- jsonlite::toJSON(result, auto_unbox = TRUE, digits = NA, null="null")
      toString(result)
   } else {
      options <- checkAnalysisOptions("jaspAnova::AnovaRepeatedMeasures", options)
      jaspTools::runAnalysis("jaspAnova::AnovaRepeatedMeasures", data, options)
   }
