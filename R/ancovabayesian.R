...
AncovaBayesianWrapper <- function(
          data = NULL,
          formula = NULL,
          isNuisance = NULL,
   ...
   defaultArgCalls <- formals(jaspAnova::AncovaBayesianWrapper)
   ...
   optionsWithFormula <- c("isNuisance", "singleModelTerms")
   for (name in optionsWithFormula) {
      if ((name %in% optionsWithFormula) && inherits(options[[name]], "formula")) options[[name]] = deparse1(options[[name]])   }

   if (jaspResultsCalledFromJasp()) {
      result <- list("options" = options, "analysis"="jaspAnova::AncovaBayesian")
      result <- jsonlite::toJSON(result, auto_unbox = TRUE, digits = NA, null="null")
      toString(result)
   } else {
      options <- checkAnalysisOptions("jaspAnova::AncovaBayesian", options)
      jaspTools::runAnalysis("jaspAnova::AncovaBayesian", data, options)
   }
