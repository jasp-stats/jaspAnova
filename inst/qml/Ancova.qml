...
Form
{
	...
	VariablesForm
	{
		...
		AssignedVariablesList	{ name: "wlsWeights";		title: qsTr("WLS Weights");	info: qsTr("Weighted Least Squares, here the variable specifying which points have more weight and are therefore considered more informative can be selected. For this last option it is important to know the weights a priori. This option is primarily used when the errors are heteroskedastic, meaning they are not equally distributed accross levels of the independent variable.")	;	allowedColumns: ["scale"]; minNumericLevels: 2;		singleVariable: true	}
	}
