...
AnovaBayesianWrapper <- function(
   ...
   defaultArgCalls <- formals(jaspAnova::AnovaBayesianWrapper)
   ...
   if (jaspResultsCalledFromJasp()) {
      result <- list("options" = options, "analysis"="jaspAnova::AnovaBayesian")
      result <- jsonlite::toJSON(result, auto_unbox = TRUE, digits = NA, null="null")
      toString(result)
   } else {
      options <- checkAnalysisOptions("jaspAnova::AnovaBayesian", options)
      jaspTools::runAnalysis("jaspAnova::AnovaBayesian", data, options)
   }
