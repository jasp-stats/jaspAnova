...
AncovaWrapper <- function(
   ...
   defaultArgCalls <- formals(jaspAnova::AncovaWrapper)
   ...
   if (jaspResultsCalledFromJasp()) {
      result <- list("options" = options, "analysis"="jaspAnova::Ancova")
      result <- jsonlite::toJSON(result, auto_unbox = TRUE, digits = NA, null="null")
      toString(result)
   } else {
      options <- checkAnalysisOptions("jaspAnova::Ancova", options)
      jaspTools::runAnalysis("jaspAnova::Ancova", data, options)
   }
